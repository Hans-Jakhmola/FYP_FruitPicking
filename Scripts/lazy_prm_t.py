...
from pybullet_planning.motion_planners.lazy_prm import lazy_prm
...
# Weighted joint-space distance function used by Lazy PRM for path-cost calculations.
DISTANCE_FN = pp.get_distance_fn(env.ur5, ARM_JOINTS, weights=JOINT_WEIGHTS)

# Joint-space interpolation function used by Lazy PRM and path smoothing.
...
# It uses pybullet_planning's path cost with the weighted joint distance function.
def path_cost(
    path,
):
...
# Builds the collision checker used by Lazy PRM and IK filtering.
...
def build_collision_fn(
    ...
    # Build the project-specific collision checker used by IK and Lazy PRM.
...
def solve_collision_free_ik_seeded(
    ...
    with RenderLock():  # lock rendering while moving the robot for IK attempts to avoid visible teleports in the GUI.
        try:
...
# This cheap direct path is tried before Lazy PRM to avoid unnecessary planning when the path is simple.
def try_direct_path(
    q_start,
    q_goal,
    collision_fn,
    ...
    # Check a straight-line joint path before running Lazy PRM.
    path = [list(q_start)] + list(DIRECT_EXTEND_FN(q_start, q_goal))
    return path if all(not collision_fn(q) for q in path) else None


# Plans a collision-free joint path using Lazy PRM.
# It first checks a direct path, then runs several Lazy PRM attempts and returns the smoothed path with the lowest cost.
def plan_path(
    q_start,
    q_goal,
    collision_fn,
    ...
    # Plan a collision-free joint-space path using Lazy PRM.

    # A direct path is checked first. If that fails, several Lazy PRM attempts are
    # run and the smoothed path with the lowest weighted joint cost is returned.

    if q_goal is None:
        return None

    q_start = [float(x) for x in np.asarray(q_start, dtype=float).reshape(-1)[:6]]
    q_goal = [float(x) for x in np.asarray(q_goal, dtype=float).reshape(-1)[:6]]

    def biased_sample(
    ):
        # Lazy PRM benefits from samples around home, the start-goal corridor and a lifted carry posture.
        q_home = np.asarray(HOME_JOINTS, dtype=float)
        qs = np.asarray(q_start, dtype=float)
        qg = np.asarray(q_goal, dtype=float)

        for _ in range(80):
            r = np.random.rand()

            if r < 0.30:
                q = q_home + np.random.normal(0.0, 0.55, size=6)
            elif r < 0.65:
                alpha = np.random.rand()
                q = (1.0 - alpha) * qs + alpha * qg
                q += np.random.normal(0.0, 0.35, size=6)
            elif r < 0.85:
                centre = np.array([0.0, -1.2, 1.8, -1.4, 0.0, 0.0], dtype=float)
                q = centre + np.random.normal(0.0, 0.55, size=6)
            else:
                q = np.random.uniform(JOINT_LOWER, JOINT_UPPER)

            q = np.clip(q, JOINT_LOWER, JOINT_UPPER).tolist()
    ...
    with RenderLock():
        ...
        try:
            ...
            try:
                # Run independent Lazy PRM attempts and keep the lowest-cost smoothed path.
                for attempt_idx in range(attempts):
                    ...
                    try:
                        ...
                        print(f"  [Lazy PRM] attempt {attempt_idx + 1} exception: {exc}")
                        continue

                    if result is None:
                        continue

                    # lazy_prm may return either a path or a PRMSolution-like object/tuple.
                    if hasattr(result, "path"):
                        path = result.path
                    elif (
                        isinstance(result, (tuple, list))
                        and len(result) >= 5
                        and isinstance(result[2], set)
                    ):
                        path = result[0]
                    else:
                        path = result
                    ...
                    for q in path:
                        ...
                        clean_path.append([float(x) for x in arr[:6]])

                    if bad_path or len(clean_path) < 2:
                        continue
                    ...
                    if smoothed is None or len(smoothed) < 2:
                        smoothed = path

                    if any(collision_fn(q) for q in smoothed):
                        continue
...
def close_gripper(
    ...
    # Close the gripper, detect contact and attach the fruit to the gripper.
    attached = False
...
def generate_fruit_placements(
    ...
    # Generate the fixed benchmark fruit positions used in the experiment.
    z_start = 0.35
    z_end = 0.62
    n_per_side = 10
...
def run_cycle(
    ...
    try:
        ...
        print("  [2] Planning to pre-grasp (Lazy PRM)...")
        t1 = time.time()
        with RenderLock():
        ...
        print("  [6] Planning carry path (Lazy PRM)...")
        t2 = time.time()
        with RenderLock():
...
if __name__ == "__main__":
    CSV_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "lazy_prm_results.csv")
    all_results = []
