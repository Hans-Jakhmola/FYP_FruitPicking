...
ur5 = p.loadURDF('Models/URDF/ur5_robotiq_85.urdf',basePosition=[0, 0, 0],useFixedBase=True) #load ur5 robot at 0,0,0 with a base fixed to the plane
...
tree = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=treecollision, baseVisualShapeIndex=treeshape, basePosition=[0.8, -0.1, 0], baseOrientation=orientation) #spawn in tree multibody using visual component, collision and base position. tree will be static so mass is 0

ur5_start_conf = [0,-1.65715,1.71108,-1.62348,0,0,0,0,0,0,0,0] #home configuration first 6 indexes are joints on the robot rest are related to the robotiq-85 gripper
